...
import exporters.svg_exporter as Exporter
import math
...
class MyPaintArea(QWidget):
    def __init__(self, parent):
        ...
        self.deform_params = None
        self.use_deformation = False

    def apply_deformation_and_subdivision(self, n_subdivisions=3, deform_params=None):
        self.n_subdivisions = n_subdivisions
        self.deform_params = deform_params or {'a': 30, 'b': 1 / 200, 'c': 0}
    ...
    def paintEvent(self, event):
        ...
        if self.use_deformation:
            tris = self.scene.all_triangles(
                n_subdivisions=self.n_subdivisions,
                deform=True,
                deform_params=self.deform_params
            )
            for tri in tris:
                polygon = QPolygonF([QPointF(*p) for p in tri])
                painter.setPen(Qt.NoPen)
                painter.setBrush(QColor(100, 200, 255))
                painter.drawPolygon(polygon)
        else:
            # Draw all shapes in their "normal" form
            ...
            if self.drawing and self.start_point and self.temp_end and self.current_tool:
                ...
                if self.current_tool == "Rectangle":
                    ...
                    outer_r = min(abs(self.temp_end.x() - self.start_point.x()),
                                  abs(self.temp_end.y() - self.start_point.y())) / 2
                    inner_r = outer_r / 2.5
                    points = []
                    for i in range(10):
                        angle = pi / 2 + i * pi / 5
    ...
    def export_svg(self, filename):
        tris = []
        for shape in self.scene.shapes:
            s_tris = shape.describe_triangles(self.n_subdivisions)
            if self.use_deformation and self.deform_params:
                s_tris = deform_triangles(s_tris, **self.deform_params)
            tris.extend(s_tris)

        width, height = self.scene.canvas_width, self.scene.canvas_height
        svg = [f'<svg width="{width}" height="{height}" xmlns="http://www.w3.org/2000/svg">']
        for tri in tris:
            points = " ".join([f"{x},{y}" for (x, y) in tri])
            # You can use a color from the shape or a fixed color
            svg.append(f'<polygon points="{points}" fill="skyblue" stroke="none"/>')
        svg.append('</svg>')
